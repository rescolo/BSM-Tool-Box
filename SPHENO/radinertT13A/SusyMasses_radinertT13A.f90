...
Module SusyMasses_radinertT13A 
...
& ZUR,ZDL,ZEL,VSs,ZUL,Vv,ZvN,ZW,ZZ,v,g1,g2,g3,Lam,LSH,Yu,Yd,Ye,YR3,YR1,YR4,              & 
& Mn,MDF,mu2,MS2,GenerationMixing,kont)
...
Complex(dp),Intent(out) :: ZDR(3,3),ZER(3,3),ZUR(3,3),ZDL(3,3),ZEL(3,3),VSs(3,3),ZUL(3,3),Vv(3,3),               & 
& ZvN(3,3),ZW(2,2)
...
Complex(dp) :: MFreC 
Iname = Iname + 1 
NameOfUnit(Iname) = 'TreeMassesradinert/T13A'
...
MFv2 = MFv**2 
Call CalculateMSs(MS2,LSH,v,VSs,MSs,MSs2,kont)
...
& YR4,Mn,MDF,mu2,MS2,GenerationMixing,kont)
...
MFv2 = MFv**2 
Call CalculateMSsEffPot(MS2,LSH,v,VSs,MSs,MSs2,kont)
...
Subroutine CalculateMSs(MS2,LSH,v,VSs,MSs,MSs2,kont)
...
Real(dp), Intent(out) :: MSs(3), MSs2(3) 
Complex(dp), Intent(out) :: VSs(3,3) 
...
Iname = Iname + 1 
NameOfUnit(Iname) = 'CalculateMSs'
 
mat(1,1) = 0._dp 
mat(1,1) = mat(1,1)+v**2*LSH(1,1)
mat(1,1) = mat(1,1)+MS2(1,1)
mat(1,2) = 0._dp 
mat(1,2) = mat(1,2)+(v**2*LSH(1,2))/2._dp
mat(1,2) = mat(1,2)+(v**2*LSH(2,1))/2._dp
mat(1,2) = mat(1,2)+MS2(1,2)/2._dp
mat(1,2) = mat(1,2)+MS2(2,1)/2._dp
mat(1,3) = 0._dp 
mat(1,3) = mat(1,3)+(v**2*LSH(1,3))/2._dp
mat(1,3) = mat(1,3)+(v**2*LSH(3,1))/2._dp
mat(1,3) = mat(1,3)+MS2(1,3)/2._dp
mat(1,3) = mat(1,3)+MS2(3,1)/2._dp
mat(2,2) = 0._dp 
mat(2,2) = mat(2,2)+v**2*LSH(2,2)
mat(2,2) = mat(2,2)+MS2(2,2)
mat(2,3) = 0._dp 
mat(2,3) = mat(2,3)+(v**2*LSH(2,3))/2._dp
mat(2,3) = mat(2,3)+(v**2*LSH(3,2))/2._dp
mat(2,3) = mat(2,3)+MS2(2,3)/2._dp
mat(2,3) = mat(2,3)+MS2(3,2)/2._dp
mat(3,3) = 0._dp 
mat(3,3) = mat(3,3)+v**2*LSH(3,3)
mat(3,3) = mat(3,3)+MS2(3,3)
...
Call EigenSystem(mat,MSs2,VSs,ierr,test) 
...
Do i1=1,3
  If (Abs(MSs2(i1)).Le.MaxMassNumericalZero) MSs2(i1) = 1.E-10_dp 
  If (MSs2(i1).ne.MSs2(i1)) Then 
      Write(*,*) 'NaN appearing in '//NameOfUnit(Iname) 
      Call TerminateProgram 
    End If 
  If (MSs2(i1).Ge.0._dp) Then 
  MSs(i1)=Sqrt(MSs2(i1) ) 
  Else 
    If (ErrorLevel.Ge.0) Then 
      Write(10,*) 'Warning from Subroutine '//NameOfUnit(Iname) 
      Write(10,*) 'a mass squarred is negative: ',i1,MSs2(i1) 
    End If 
  MSs = 1._dp 
     Write(ErrCan,*) 'Warning from routine '//NameOfUnit(Iname) 
     Write(ErrCan,*) 'in the calculation of the masses' 
     Write(ErrCan,*) 'occurred a negative mass squared!' 
     Write(ErrCan,*) i1,MSs2(i1) 
     Write(*,*) 'Warning from routine '//NameOfUnit(Iname) 
     Write(*,*) 'in the calculation of the masses' 
     Write(*,*) 'occurred a negative mass squared!' 
     Write(*,*) i1,MSs2(i1) 
...
Subroutine CalculateMSsEffPot(MS2,LSH,v,VSs,MSs,MSs2,kont)
...
Real(dp) :: MSs2temp(3), Q2 
Complex(dp) :: VSstemp(3,3), VSstemp2(3,3) 
...
! Fix order
  VSstemp2=VSs
Do i1=1,3
  pos=Maxloc(Abs(VSstemp2(i1,:)),1)
  VSstemp(pos,:)=VSs(i1,:)
  MSs2temp(pos)=MSs2(i1)
  VSstemp2(:,pos)=0._dp
End do
  MSs2 = MSs2temp
  VSs = VSstemp
...
End Subroutine CalculateMSsEffPot 
...
& MVZ,MVZ2,TW,ZDR,ZER,ZUR,ZDL,ZEL,ZUL,Vv,ZW,ZZ,v,g1,g2,g3,Lam,LSH,Yu,Yd,Ye,              & 
& YR3,YR1,YR4,Mn,MDF,mu2,MS2,GenerationMixing,kont)
...
& MFv,MFv2,Mhh,Mhh2,MHp,MHp2,MNv0,MNv02,MSs,MSs2,MVWp,MVWp2,MVZ,MVZ2,TW,ZDR,             & 
& ZER,ZUR,ZDL,ZEL,VSs,ZUL,Vv,ZvN,ZW,ZZ,kont)
...
End Module SusyMasses_radinertT13A 
