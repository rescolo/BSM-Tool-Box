...
Module Ftc3Decays_SimplifiedDMTFDM 
...
Subroutine FtcThreeBodyDecay(n_in,MAh,MAh2,MFd,MFd2,MFe,MFe2,MFt0,MFt02,              & 
& MFtc,MFtc2,MFu,MFu2,MFv,MFv2,Mhh,Mhh2,MHp,MHp2,MVWp,MVWp2,MVZ,MVZ2,TW,ZDR,             & 
& ZER,ZUR,ZDL,ZEL,ZUL,UV,ZW,ZZ,g1,g2,g3,Lam,Yu,Yd,Ye,MTF,Mu,v,gTVWp,gFtcFt0cFuFd,        & 
& gFtcFt0cFvFe,epsI,deltaM,CheckRealStates,gT,gPartial,BR)
...
Complex(dp) :: cplcFtcFt0cVWpL,cplcFtcFt0cVWpR,cplcFuFdVWpL(3,3),cplcFuFdVWpR(3,3),cplcFvFeVWpL(3,3),& 
& cplcFvFeVWpR(3,3)
...
Real(dp),Intent(inout) :: gFtcFt0cFuFd(1,1,3,3),gFtcFt0cFvFe(1,1,3,3)

Real(dp),Intent(in) :: gTVWp

Real(dp) :: gFtcFt0cFuFdi(1,3,3),gFtcFt0cFvFei(1,3,3)
...
Call CouplingsFor_Ftc_decays_3B(MFtc,i_run,MAh,MAh2,MFd,MFd2,MFe,MFe2,MFt0,           & 
...
& cplcFtcFt0cVWpR,cplcFuFdVWpL,cplcFuFdVWpR,cplcFvFeVWpL,cplcFvFeVWpR,deltaM)
...
gT = gT + Sum(gFtcFt0cFuFdi) 
 
gFtcFt0cFvFei = 0._dp 
Call FtcToFt0cFvFe(i_run,MFt0,MFv,MFe,MVWp,MFtc,cplcFtcFt0cVWpL,cplcFtcFt0cVWpR,      & 
& cplcFvFeVWpL,cplcFvFeVWpR,IntegralVs,IntegralVVss,NVs,NVVss,gTVWptemp,deltaM,          & 
& epsI,check,gFtcFt0cFvFei)

gFtcFt0cFvFe(i_run,:,:,:) = gFtcFt0cFvFei 
gT = gT + Sum(gFtcFt0cFvFei) 
...
Do gt1=1,1
  Do gt2=1,3
    Do gt3=1,3
gPartial(i1,n_length)= gFtcFt0cFvFe(i1,gt1,gt2,gt3)
n_length=n_length+1
     End Do 
  End Do 
End Do 
...
Real(dp),Intent(in) :: MFt0,MFu(3),MFd(3),MVWp,MFtc
...
If(Abs(MFtc).gt.(Abs(MFd(gt3))+Abs(MFu(gt2))+Abs(MFt0))) Then 
!-------------- 
!  conj[VWp] 
!-------------- 
Isum = Isum + 1 
...
coup(2) = Conjg(cplcFtcFt0cVWpL) 
coup(1) = Conjg(cplcFtcFt0cVWpR) 
...
End If
       End Do 
     End Do 
  g = oo512pi3 / Abs(MFtc)**3*g
End Subroutine FtcToFt0cFuFd 
 
 
Subroutine FtcToFt0cFvFe(iIN,MFt0,MFv,MFe,MVWp,MFtc,cplcFtcFt0cVWpL,cplcFtcFt0cVWpR,  & 
& cplcFvFeVWpL,cplcFvFeVWpR,IntegralVs,IntegralVVss,NVs,NVVss,gTVWp,deltaM,              & 
& epsI,check,g,WriteContributions)

Implicit None 
 
Real(dp),Intent(in) :: MFt0,MFv(3),MFe(3),MVWp,MFtc

Complex(dp),Intent(in) :: cplcFtcFt0cVWpL,cplcFtcFt0cVWpR,cplcFvFeVWpL(3,3),cplcFvFeVWpR(3,3)
...
If(Abs(MFtc).gt.(Abs(MFe(gt3))+Abs(MFv(gt2))+Abs(MFt0))) Then 
!-------------- 
!  conj[VWp] 
!-------------- 
Isum = Isum + 1 
...
If (resD.ne.resD) Then 
Write(*,*) "NaN appearing in the following diagrams: " 
Write(*,*) "Ftc->Ft0 cFv Fe Propagator: conj[VWp]" 
Write(*,*)  "M_in: ",m_in 
Write(*,*)  "mass: ",mass 
Write(*,*)  "coup: ",coup 
gSum(1,gt2,gt3,Isum)= 0._dp
...
End If
     ...
     End Do 
  g = oo512pi3 / Abs(MFtc)**3*g
End Subroutine FtcToFt0cFvFe 
 
 
End Module Ftc3Decays_SimplifiedDMTFDM 
