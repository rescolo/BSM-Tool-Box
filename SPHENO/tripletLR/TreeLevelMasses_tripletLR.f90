...
! ------------------------------------------------------------------------------  
! File created at 12:48 on 3.10.2016   
! ----------------------------------------------------------------------  
 
 
Module TreeLevelMasses_tripletLR 
...
Real(dp),Intent(out) :: MAh(3),MAh2(3),MdeltaRpp,MdeltaRpp2,MFc(3),MFc2(3),MFcp(2),MFcp2(2),MFcpp,            & 
...
Complex(dp),Intent(out) :: phaT,UV(6,6),ZDR(3,3),ZER(3,3),UT(3,3),UTE(2,2),ZUR(3,3),ZDL(3,3),ZEL(3,3),           & 
...
MFv2 = MFv**2 
Call CalculateMFc(M1,M23,LAMT1,LAMT2,vR,UT,MFc,kont)

MFc2 = MFc**2 
Call CalculateMFcp(M1,M23,LAMT1,LAMT2,vR,VTE,UTE,MFcp,kont)
...
& vd,vu,vR,gBL,g2,gR,g3,RHO2,RHO1,ALP1,LAM1,ALP3,ALP2,LAM4,LAM2,LAM3,LAMT1,              & 
& YDR,YL1,YQ1,LAMT2,YL2,YQ2,M1,M23,mu32,MU22,MU12,kont)
...
& UP,UT,UTE,ZUR,ZDL,ZEL,VTE,ZUL,ZH,ZW,ZZ,vd,vu,vR,gBL,g2,gR,g3,RHO2,RHO1,ALP1,           & 
& LAM1,ALP3,ALP2,LAM4,LAM2,LAM3,LAMT1,YDR,YL1,YQ1,LAMT2,YL2,YQ2,M1,M23,mu32,             & 
...
MFv2 = MFv**2 
Call CalculateMFcEffPot(M1,M23,LAMT1,LAMT2,vR,UT,MFc,kont)

MFc2 = MFc**2 
Call CalculateMFcpEffPot(M1,M23,LAMT1,LAMT2,vR,VTE,UTE,MFcp,kont)
...
Subroutine CalculateMFc(M1,M23,LAMT1,LAMT2,vR,UT,MFc,kont)
...
Integer :: j1,j2,j3,j4 
Logical :: SecondDiagonalisationNeeded 
Real(dp), Intent(out) :: MFc(3) 
Complex(dp), Intent(out) ::  UT(3,3) 
...
mat(1,1) = mat(1,1)+M1
mat(1,2) = 0._dp 
mat(1,2) = mat(1,2)+(LAMT2*vR)/2._dp
mat(1,3) = 0._dp 
mat(1,3) = mat(1,3)-(LAMT1*vR)/2._dp
mat(2,2) = 0._dp 
mat(2,3) = 0._dp 
mat(2,3) = mat(2,3)+M23
mat(3,3) = 0._dp 
...
Subroutine CalculateMFcp(M1,M23,LAMT1,LAMT2,vR,VTE,UTE,MFcp,kont)
...
Integer :: i1,i2,i3,i4, ierr 
Integer :: j1,j2,j3,j4 
Real(dp), Intent(out) :: MFcp(2) 
 Complex(dp), Intent(out) :: VTE(2,2), UTE(2,2) 
 
 Complex(dp) :: mat(2,2)=0._dp, mat2(2,2)=0._dp, phaseM 

Complex(dp) :: VTE2(2,2), UTE2(2,2) 
 
 Real(dp) :: VTE1(2,2), UTE1(2,2), test(2), MFcp2(2) 
 
 Iname = Iname + 1 
NameOfUnit(Iname) = 'CalculateMFcp'
...
mat(1,1) = mat(1,1)+M1
mat(1,2) = 0._dp 
mat(1,2) = mat(1,2)+(LAMT1*vR)/2._dp
mat(2,1) = 0._dp 
mat(2,1) = mat(2,1)-(LAMT2*vR)/2._dp
mat(2,2) = 0._dp 
mat(2,2) = mat(2,2)+M23
...
mat2 = Matmul(Transpose(Conjg(mat)),mat) 
If (Maxval(Abs(Aimag(mat2))).Eq.0._dp) Then 
Call EigenSystem(Real(mat2,dp),MFcp2,UTE1,ierr,test) 
UTE2 = UTE1 
Else 
Call EigenSystem(mat2,MFcp2,UTE2,ierr,test) 
 End If 
...
mat2 = Matmul(mat,Transpose(Conjg(mat))) 
If (Maxval(Abs(Aimag(mat2))).Eq.0._dp) Then 
Call EigenSystem (Real(mat2,dp),MFcp2,VTE1,ierr,test) 
                  
                  
VTE2 = VTE1 
Else 
Call EigenSystem(mat2,MFcp2,VTE2,ierr,test) 
...
mat2 = Matmul(Matmul( Conjg(VTE2),mat),Transpose( Conjg(UTE2))) 
Do i1=1,2
If (Abs(mat2(i1,i1)).gt.0._dp) Then 
phaseM = mat2(i1,i1) / Abs(mat2(i1,i1)) 
...
Do i1=1,2
If (Abs(UTE2(i1,i1)).gt.0._dp) Then 
phaseM = UTE2(i1,i1) / Abs(UTE2(i1,i1)) 
UTE2(i1,:) = Conjg(phaseM) *UTE2(i1,:) 
 VTE2(i1,:) = phaseM *VTE2(i1,:) 
 End if 
  If (MFcp2(i1).ne.MFcp2(i1)) Then 
      Write(*,*) 'NaN appearing in '//NameOfUnit(Iname) 
      Call TerminateProgram 
    End If 
  If (Abs(MFcp2(i1)).Le.MaxMassNumericalZero) MFcp2(i1) = Abs(MFcp2(i1))+1.E-10_dp 
  If (MFcp2(i1).Le.0._dp) Then 
    If (ErrorLevel.Ge.0) Then 
      Write(10,*) 'Warning from Subroutine '//NameOfUnit(Iname) 
      Write(10,*) 'a mass squarred is negative: ',i1,MFcp2(i1) 
      Write(*,*) 'Warning from Subroutine '//NameOfUnit(Iname) 
      Write(*,*) 'a mass squarred is negative: ',i1,MFcp2(i1) 
      Call TerminateProgram 
    End If 
     Write(ErrCan,*) 'Warning from routine '//NameOfUnit(Iname) 
     Write(ErrCan,*) 'in the calculation of the masses' 
     Write(ErrCan,*) 'occurred a negative mass squared!' 
     Write(ErrCan,*) i1,MFcp2(i1) 
     Write(*,*) 'Warning from routine '//NameOfUnit(Iname) 
     Write(*,*) 'in the calculation of the masses' 
     Write(*,*) 'occurred a negative mass squared!' 
     Write(*,*) i1,MFcp2(i1) 
...
If (ierr.Ne.0.) Then 
  Write(ErrCan,*) 'Warning from Subroutine CalculateMFcp, ierr =',ierr 
  kont = ierr 
  Iname = Iname - 1 
  Return 
End If 
...
Subroutine CalculateMFcEffPot(M1,M23,LAMT1,LAMT2,vR,UT,MFc,kont)
...
Subroutine CalculateMFcpEffPot(M1,M23,LAMT1,LAMT2,vR,VTE,UTE,MFcp,kont)
...
End Subroutine CalculateMFcpEffPot 
...
& vu,vR,gBL,g2,gR,g3,RHO2,RHO1,ALP1,LAM1,ALP3,ALP2,LAM4,LAM2,LAM3,LAMT1,YDR,             & 
& YL1,YQ1,LAMT2,YL2,YQ2,M1,M23,mu32,MU22,MU12,GenerationMixing,kont)
...
& MFcpp,MFcpp2,MFd,MFd2,MFe,MFe2,MFu,MFu2,MFv,MFv2,Mhh,Mhh2,MHpm,MHpm2,MVWLm,            & 
& MVWLm2,MVWRm,MVWRm2,MVZ,MVZ2,MVZR,MVZR2,phaT,PhiW,UV,TW,UC,ZDR,ZER,UP,UT,              & 
& UTE,ZUR,ZDL,ZEL,VTE,ZUL,ZH,ZW,ZZ,kont)
...
End Module TreeLevelMasses_tripletLR 
