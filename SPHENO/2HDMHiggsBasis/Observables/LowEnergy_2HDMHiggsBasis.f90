...
Module LowEnergy_2HDMHiggsBasis 
Use Control 
...
Subroutine Gminus2(Ifermion,MAh,MAh2,MetI,MetI2,Metp,Metp2,MFe,MFe2,Mhh,              & 
& Mhh2,MHp,MHp2,cplcFeFeAhL,cplcFeFeAhR,cplcFeFeetIL,cplcFeFeetIR,cplcFvFeetpL,          & 
& cplcFvFeetpR,cpletpcetpVP,cplcFeFehhL,cplcFeFehhR,cplcFvFeHpL,cplcFvFeHpR,             & 
& cplcFeFeVPL,cplcFeFeVPR,cplcFeFvcetpL,cplcFeFvcetpR,cplcFeFvcHpL,cplcFeFvcHpR,         & 
& cplHpcHpVP,a_mu)

Real(dp),Intent(in)  :: MAh,MAh2,MetI,MetI2,Metp,Metp2,MFe(3),MFe2(3),Mhh(2),Mhh2(2),MHp,MHp2

Complex(dp),Intent(in)  :: cplcFeFeAhL(3,3),cplcFeFeAhR(3,3),cplcFeFeetIL(3,3),cplcFeFeetIR(3,3),cplcFvFeetpL(3,3),& 
& cplcFvFeetpR(3,3),cpletpcetpVP,cplcFeFehhL(3,3,2),cplcFeFehhR(3,3,2),cplcFvFeHpL(3,3), & 
& cplcFvFeHpR(3,3),cplcFeFeVPL(3,3),cplcFeFeVPR(3,3),cplcFeFvcetpL(3,3),cplcFeFvcetpR(3,3),& 
& cplcFeFvcHpL(3,3),cplcFeFvcHpR(3,3),cplHpcHpVP
...
chargefactor = 1 
Do i1= 1,3
  If ((0._dp.gt.mz2).Or.(Metp2.gt.mz2).Or.(Metp2.gt.mz2)) Then
coup1L = cplcFeFvcetpL(gt1,i1)
coup1R = cplcFeFvcetpR(gt1,i1)
coup2L = cplcFvFeetpL(i1,gt2)
coup2R = cplcFvFeetpR(i1,gt2)
ratio = Metp2/0._dp
 If ((ratio.eq.ratio).and.(ratio.lt.1.0E+30_dp).and.(ratio.gt.1.0E-30_dp)) Then 
...
Subroutine LeptonEDM(Ifermion,MAh,MAh2,MetI,MetI2,Metp,Metp2,MFe,MFe2,Mhh,            & 
& Mhh2,MHp,MHp2,MVWp,MVWp2,MVZ,MVZ2,cplcFeFeAhL,cplcFeFeAhR,cplcFeFeetIL,cplcFeFeetIR,   & 
& cplcFvFeetpL,cplcFvFeetpR,cpletpcetpVP,cplcFeFehhL,cplcFeFehhR,cplcFvFeHpL,            & 
& cplcFvFeHpR,cplcFeFeVPL,cplcFeFeVPR,cplcFeFeVZL,cplcFeFeVZR,cplcFvFecVWpL,             & 
& cplcFvFecVWpR,cplcFeFvVWpL,cplcFeFvVWpR,cplcFeFvcetpL,cplcFeFvcetpR,cplcFeFvcHpL,      & 
& cplcFeFvcHpR,cplHpcHpVP,cplcVWpVPVWp,EDM)

Implicit None
Real(dp),Intent(in)  :: MAh,MAh2,MetI,MetI2,Metp,Metp2,MFe(3),MFe2(3),Mhh(2),Mhh2(2),MHp,MHp2,MVWp,           & 
& MVWp2,MVZ,MVZ2

Complex(dp),Intent(in)  :: cplcFeFeAhL(3,3),cplcFeFeAhR(3,3),cplcFeFeetIL(3,3),cplcFeFeetIR(3,3),cplcFvFeetpL(3,3),& 
& cplcFvFeetpR(3,3),cpletpcetpVP,cplcFeFehhL(3,3,2),cplcFeFehhR(3,3,2),cplcFvFeHpL(3,3), & 
& cplcFvFeHpR(3,3),cplcFeFeVPL(3,3),cplcFeFeVPR(3,3),cplcFeFeVZL(3,3),cplcFeFeVZR(3,3),  & 
& cplcFvFecVWpL(3,3),cplcFvFecVWpR(3,3),cplcFeFvVWpL(3,3),cplcFeFvVWpR(3,3),             & 
...
coup2L = cplcFvFecVWpL(i1,gt2)
coup2R = cplcFvFecVWpR(i1,gt2)
End if 
   End Do
...
Subroutine DeltaRho(MAh,MAh2,MetI,MetI2,Metp,Metp2,MFd,MFd2,MFe,MFe2,MFu,             & 
& MFu2,Mhh,Mhh2,MHp,MHp2,MVWp,MVWp2,MVZ,MVZ2,cplAhAhcVWpVWp,cplAhAhVZVZ,cplAhcHpcVWp,    & 
...
Complex(dp),Intent(in)  :: cplAhAhcVWpVWp,cplAhAhVZVZ,cplAhcHpcVWp,cplAhhhVZ(2),cplcetpcVWpVP,cplcetpcVWpVZ,     & 
& cplcFdFdVZL(3,3),cplcFdFdVZR(3,3),cplcFeFeVZL(3,3),cplcFeFeVZR(3,3),cplcFuFdcVWpL(3,3),& 
& cplcFuFdcVWpR(3,3),cplcFuFuVZL(3,3),cplcFuFuVZR(3,3),cplcFvFecVWpL(3,3),               & 
& cplcFvFecVWpR(3,3),cplcFvFvVZL(3,3),cplcFvFvVZR(3,3),cplcgAgWpcVWp,cplcgWCgAcVWp,      & 
& cplcgWCgWCVZ,cplcgWCgZcVWp,cplcgWpgWpVZ,cplcgZgWpcVWp,cplcHpcVWpVP,cplcHpcVWpVZ,       & 
...
& cplcVWpVWpVZVZ2,cplcVWpVWpVZVZ3,cpletIcetpcVWp,cpletIetIcVWpVWp,cpletIetIVZVZ,         & 
& cpletIhhVZ(2),cpletpcetpcVWpVWp,cpletpcetpVZ,cpletpcetpVZVZ,cpletpVWpVZ,               & 
& cplhhcetpcVWp(2),cplhhcHpcVWp(2),cplhhcVWpVWp(2),cplhhhhcVWpVWp(2,2),cplhhhhVZVZ(2,2), & 
& cplhhVZVZ(2),cplHpcHpcVWpVWp,cplHpcHpVZ,cplHpcHpVZVZ,cplHpVWpVZ
...
Call Pi1LoopVZ(0._dp,Mhh,Mhh2,MAh,MAh2,MetI,MetI2,Metp,Metp2,MVWp,MVWp2,              & 
...
Call Pi1LoopVWp(0._dp,MHp,MHp2,MAh,MAh2,Metp,Metp2,MetI,MetI2,MFu,MFu2,               & 
...
End Function C1C2 
End Module LowEnergy_2HDMHiggsBasis 
