...
class SM(object):
    ...
    def __init__(self,blocks=['modsel','sminputs'],ignorenobr=True,ignorenomass=True):
        spcskel=''
        for b in blocks:
            spcskel=spcskel+'block '+b+"\n"
        self.LHA=pyslha.readSLHA(spcskel,ignorenobr=ignorenobr,ignorenomass=ignorenomass)
        if self.LHA.blocks.has_key('SMINPUTS'):
        ...
        if self.LHA.blocks.has_key('SPHENOINPUT'):
            self.LHA.blocks['SPHENOINPUT'][  1]='-1              # error level'
            self.LHA.blocks['SPHENOINPUT'][  2]=' 0              # SPA conventions'
            self.LHA.blocks['SPHENOINPUT'][ 11]='1               # calculate branching ratios'
            self.LHA.blocks['SPHENOINPUT'][ 13]='1               # 3-Body decays: none (0), fermion (1), scalar (2), both (3)'
            self.LHA.blocks['SPHENOINPUT'][ 14]='0               # Run couplings to scale of decaying particle'
            ...
            self.LHA.blocks['SPHENOINPUT'][ 31]='-1              # fixed GUT scale (-1: dynamical GUT scale)'
            self.LHA.blocks['SPHENOINPUT'][ 32]='0               # Strict unification'
            self.LHA.blocks['SPHENOINPUT'][ 34]='1.000E-04       # Precision of mass calculation'
            self.LHA.blocks['SPHENOINPUT'][ 35]='40              # Maximal number of iterations'
            self.LHA.blocks['SPHENOINPUT'][ 37]='1               # Set Yukawa scheme'
            self.LHA.blocks['SPHENOINPUT'][ 38]='2               # 1- or 2-Loop RGEs'
            self.LHA.blocks['SPHENOINPUT'][ 50]='0               # Majorana phases: use only positive masses'
            self.LHA.blocks['SPHENOINPUT'][ 51]='0               # Write Output in CKM basis'
            self.LHA.blocks['SPHENOINPUT'][ 52]='1               # Write spectrum in case of tachyonic states'
            self.LHA.blocks['SPHENOINPUT'][ 55]='0               # Calculate one loop masses'
            self.LHA.blocks['SPHENOINPUT'][ 57]='1               # Calculate low energy constraints'
            self.LHA.blocks['SPHENOINPUT'][ 65]='0               # Solution tadpole equation'
            self.LHA.blocks['SPHENOINPUT'][ 75]='1               # Write WHIZARD files'
            self.LHA.blocks['SPHENOINPUT'][ 76]='1               # Write HiggsBounds file'
            self.LHA.blocks['SPHENOINPUT'][ 86]='0.              # Maximal width to be counted as invisible in Higgs decays; -1: only LSP'
            self.LHA.blocks['SPHENOINPUT'][510]='0.              # Write tree level values for tadpole solutions'
            self.LHA.blocks['SPHENOINPUT'][515]='0               # Write parameter values at GUT scale'
            self.LHA.blocks['SPHENOINPUT'][525]='0.              # Write loop contributions to diphoton decay of Higgs'
        if self.LHA.blocks.has_key('MODSEL'):
            self.LHA.blocks['MODSEL'][1]='0              #  1/0: High/low scale input'
            self.LHA.blocks['MODSEL'][2]='1              # Boundary Condition'
            self.LHA.blocks['MODSEL'][6]='1              # Generation Mixing'
            self.LHA.blocks['MODSEL'][12]='1000.          # Renormalization scale'
