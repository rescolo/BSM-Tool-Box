          ...
          If[FreeQ[vevEx,vevDef[[i,2,1]]],
            ...
            TadpoleEquations = 
              Join[TadpoleEquations, {Simplify[
                    makeSumAll[
                      CalcDelta[
                          DPTad[pot,vevDef[[i,2,1]],1]/.Map[vacHead,
...
CalcTadpoleEquations[pot_,ES_]:=
    Block[{TadpoleEquations,vevEx={},temp,TadWF={}},
      potSave=pot;
      ...
      For[i=1,i\[LessEqual] Length[SA`ScalarHF[ES]],
        temp=
          Simplify[
            makeSumAll[
              CalcDelta[
                  DPTad[pot,SA`ScalarHF[ES][[i]],1]/.Map[vacHead,
                          vacuum]/.Mom[_]\[Rule]0 /.zero[a_]\[Rule]0*
                        a  ]/.sum[gt1,a___]\[Rule]1]];
        
        savetad=
          Join[savetad,{DPTad[pot,SA`ScalarHF[ES][[i]],1]/.Map[vacHead,
                      vacuum]/.Mom[_]\[Rule]0 /.zero[a_]\[Rule]0*a }];
      ...
      If[SA`CPViolationHiggsSector===True,
        For[i=1,i\[LessEqual] Length[SA`PseudoScalarHF[ES]],
            temp=
              Simplify[
                makeSumAll[
                  CalcDelta[
                      DPTad[pot,SA`PseudoScalarHF[ES][[i]],1]/.Map[vacHead,
                              vacuum]/.Mom[_]\[Rule]0 /.zero[a_]\[Rule]0*
                            a  ]/.sum[gt1,a___]\[Rule]1]];
            TadpoleEquations = Join[TadpoleEquations, {temp}];
...
DPTad[term_,particle_,t_]:=Block[{part,i},
      ...
      If[(Length[
                  getIndizesOld[
                    particle]]\[Equal]0 && (OnesChecked\[Equal]True)) || 
          Depth[particle[{gen1}]]\[Equal]1,
        Return[D[term,part]];,
        Return[
            Plus@@Table[
                D[term,part /. subGC[i]] /. subIndFinal[i,t],{i,1,genMax}]];
        ]; 
      ];


SolveTadpoleEquations[Eigenstates_,parameters_]:=
    Block[{i,j,k,off,i1,i2,temp={},vevs={},basis={},vevWithGen={},TEqu={},
        subReal = {},subPhases={},
      ...
      For[k=1,k\[LessEqual]Length[NameOfStates],
      ...
      For[i=1,i\[LessEqual]Length[temp],
        For[j=1,j\[LessEqual]getGenALL[vevs[[i,1]]],
      ...
      vevTemp=Map[
          Sort[#,(Position[basis,vevs[[Position[vevs,#][[1,1]]]][[4,1]]][[1,
                        3]]<Position[basis,
                        vevs[[Position[vevs,#2][[1,1]]]][[4,1]]][[1,3]])&]&,
          listVEVseparated];
      subTadpoleNumber={};
      For[i=1,i\[LessEqual]Length[ScalarsForTadpoles],
        For[j=1,j\[LessEqual]Length[listVEVseparated[[i]]],
          If[getGenALL[vevTemp[[i,j]]]===1,
            
            subTadpoleNumber = 
              Join[subTadpoleNumber,{vevTemp[[i,j]]\[Rule]vcount}];
            vcount++;,
            For[k=1,k\[LessEqual]getGenALL[vevTemp[[i,j]]],
                
                subTadpoleNumber = 
                  Join[subTadpoleNumber,{vevTemp[[i,j]][k]\[Rule]vcount}];
                vcount++;
                k++;];
            ];
          j++;];
      ...
      For[i=1,i\[LessEqual]Length[TEqu],
        EquLoop = 
          Join[EquLoop,{TEqu[[i]]-
                  Tad1Loop[vevWithGen[[i]]  /. subTadpoleNumber]\[Equal]0}];
      ...
      For[i=1,i\[LessEqual]Length[parameters],
        ...
        subSPhenoForm=
          Join[subSPhenoForm,{temp\[Rule] 
                ToExpression[getOutputNameParameter[temp]]}];
        subSPhenoFormBack=
          Join[subSPhenoFormBack,{ToExpression[
                  getOutputNameParameter[temp]]\[Rule]temp}];
        
        If[FreeQ[EquTree,conj[parameters[[i]]]]===False,
          
          If[getDimParameter[parameters[[i]]]=!={1} && 
                getDimParameter[parameters[[i]]]=!={},
              
              subGenParameters = 
                Join[subGenParameters,{parameters[[i]]\[Rule] 
                      Table[parameters[[i]][k],{k,1,
                          getDimParameter[parameters[[i]]][[1]]}]}];
              For[k=1,k\[LessEqual] getDimSPheno[parameters[[i]]][[1]],
                
                ph=ToExpression[
                    "Signum"<>ToString[
                        getOutputNameParameter[parameters[[i]]]]];
                
                subPhases = 
                  Join[subPhases,{parameters[[i]][k] \[Rule] 
                        parameters[[i]][k]*Exp[\[ImaginaryI] ph[k]]}];
                
                subReal = 
                  Join[subReal,{conj[parameters[[i]][k]]\[Rule]
                        parameters[[i]][k], conj[ph[k]]\[Rule]ph[k]}];
              ...
              subPhases = 
                Join[subPhases,{parameters[[i]] \[Rule] 
                      parameters[[i]]*Exp[\[ImaginaryI] ph]}];
      ...
      resLoop=
        Solve[EquLoop,
            Flatten[(parameters /. subSPhenoForm /. subGenParameters)]] /. 
          subSPhenoFormBack;
      resTree=
        Solve[EquTree,
            Flatten[(parameters /. subSPhenoForm /. subGenParameters)]]/. 
      ...
      (*
        ...
        SubSolutionsTadpolesLoop=
          Simplify[
            Map[Expand,SubSolutionsTadpolesLoop,3] //. 
                      Power[a__, Rational[-1,2]]\[Rule] 1/sqrt[a] //. 
                    Sqrt[a_]\[Rule]sqrt[a] //.  
                  sqrt[a_] sqrt[b_]\[Rule]sqrt[a b] //. 
                sqrt[a_] / sqrt[b_] \[Rule] sqrt[a/b] //. sqrt \[Rule]Sqrt];
        SubSolutionsTadpolesTree=
          Simplify[
            Map[Expand,SubSolutionsTadpolesTree,3]//. 
                      Power[a__, Rational[-1,2]]\[Rule] 1/sqrt[a] //. 
                    Sqrt[a_]\[Rule]sqrt[a] //.  
                  sqrt[a_] sqrt[b_]\[Rule]sqrt[a b] //. 
                sqrt[a_] / sqrt[b_] \[Rule] sqrt[a/b] //. sqrt \[Rule]Sqrt]; 
