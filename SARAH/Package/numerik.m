...
ReadSpectrum[file_]:=Block[{i,j,a,b,c,lval,nrgesucht,j2, list},
      ...
      ReadSpectrumFile[ToFileName[$sarahCurrentModelDir,file],
        ITSJUSTONEFILE];
      
      For[i=1,i\[LessEqual]Length[LesHouchesList],
        If[
          FreeQ[LesHouchesInput,LHBlockName[LesHouchesList[[i,1]]]]\[Equal]
            False,
                ...
                subNum = 
                  Join[subNum, 
                    Table[LesHouchesList[[i,1]]@@
                          Transpose[Drop[Transpose[real],-1]][[j]]\[Rule]
                        Last[Transpose[real]][[j]]+\[ImaginaryI] Last[
...
SortDecay[list_]:=Block[{temp={},temp1={},i},
      ...
      For[i=2,i\[LessEqual]Length[temp1],
        temp=
          Join[temp,{{FinalParticles@@Take[temp1[[i]],{3,Length[temp1[[i]]]}],
                temp1[[i,1]]}}];
        i++;];
      Return[temp];
      ];

ReadSpectrumFile[file_,splitsequence_]:=
    Block[{i,j,a,b,c,sps={}, temp,t="", return},
      ...
      While[return=!=EndOfFile, 
        ...
        If[splitsequence=!=False,
          If[return===splitsequence,
              ...
              While[t=!="BLOCK" && t=!="DECAY" && t=!=ToUpperCase[EndOfFile], 
                t=ToUpperCase[Read[in,Word]]; ];
...
ReadBlock[decay_,SplitLine_]:=Block[{i,t,temp,temp1={},j,test,first=True},
      ...
      If[decay\[Equal]True,
        ...
        width = 
          ToExpression[
            StringReplace[
              StringTake[width,{1,StringPosition[width,"#"][[1,1]]-1}],{"E"->"*10^",
                "e"->"*10^"}]];
        ];
      While[
        ToUpperCase[j]=!="BLOCK" && ToUpperCase[j]=!="DECAY" &&
          ToUpperCase[j]=!= SplitLine&& j=!=EndOfFile,
        i=j;
        If[i==="#",
          ...
          If[first=!=True &&  ToUpperCase[j]=!="DECAY", 
              temp1 = Join[
                    temp1,{ToExpression[
                        StringReplace[i,{"E"->"*10^","e"->"*10^"}]]}]; ];
          ];
        j=Read[in,Word];
        first=False;
        ];
...
MakeSubNum[list_]:=Block[{temp},
      ...
      Return[
        Flatten[Table[
            Table[ToExpression[temp[[i,1]]]@@Drop[temp[[i,j]],-1]\[Rule]Last[
                  temp[[i,j]]],{j,2,Length[temp[[i]]]}],{i,1,
              Length[temp]}]]];
      ];
...
NVal[x_Mass]:=Block[{},
      ...
      If[NumericValQ[(x /. {Mass[a_,b_]\[Rule]MassGiven[a[b]] , 
                  Mass[a_] \[Rule] MassGiven[a]} /. subNum)],
        Return[
            x /. {Mass[a_,b_]\[Rule]MassGiven[a[b]] , 
                  Mass[a_] \[Rule] MassGiven[a]} /. subNum];
        ];
      If[NumericValQ[(x /. {Mass[a_,b_]\[Rule]MassRead[a[b]] , 
                  Mass[a_] \[Rule] MassRead[a]} /. subNum)],
        Return[
            x /. {Mass[a_,b_]\[Rule]MassRead[a[b]] , 
                  Mass[a_] \[Rule] MassRead[a]} /. subNum];
        ];
      Return[x];
      ];
...
NVal[x_Width]:=Block[{},
      If[NumericValQ[(x /.  subNum)],Return[x /.subNum];];
      If[NumericValQ[(x /. {Width[a_,b_]\[Rule]WidthGiven[a[b]] , 
                  Width[a_] \[Rule] WidthGiven[a]} /. subNum)],
        Return[
            x /. {Width[a_,b_]\[Rule]WidthGiven[a[b]] ,
                  Width[a_] \[Rule] WidthGiven[a]} /. subNum];
        ];
      Return[x];
      ];
        ...
        If[WriteErrorNum,
          ...
          listNoNumeric=
            DeleteCases[
              Intersection[
                Flatten[
                  DeleteCases[Expand[result] /. Power\[Rule]Times ,
                            y_?NumericQ,2] /. Plus\[Rule]List /. 
                        Times\[Rule]List /. 
                      conj[y_]\[Rule]y /. {Sin[y_]\[Rule]y,Tan[y_]\[Rule]y,
                      Cos[y_]\[Rule]y}]],y_?((NumericValQ[#])&)];
          ...
          subNum=Join[
              subNum,{ToExpression[
                    StringDrop[ToString[V],Length[ToString[V]]-3]<>
                      "[i,j]"]\[Rule]Vtemp[[i,j]]}];
          j++;];
        subNum= Join[subNum,{Mass[M1,i] \[Rule] Sqrt[dia1[[i,i]]]}];
        subNum= Join[subNum,{Mass[M2,i] \[Rule] Sqrt[dia2[[i,i]]]}];
        i++;];
      Set[U,Utemp];
      Set[V,Vtemp];
      ];
